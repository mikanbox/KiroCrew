...
def _replay_mirrored_frame(
    ...
    try:
        ...
        # ``event`` crossed the relay boundary, so it is peer-controlled text.
        logger.debug("Relay dropped an undecodable %s frame", redact_peer_text(event)[:64])
        return
    ...
    if "key" in data:  # slot_title / session_summary carry `key`, not `slot`
        data["key"] = slot.key
    if event == "slot_clear":
        # A mirrored clear has to land on LOCAL state, not just the browser. The
        # peer cleared its own transcript, but THIS slot owns the rows the
        # dashboard reloads from and the persister writes back, so broadcasting
        # alone blanks the open view and then restores every cleared row on the
        # next reload or slot switch. Mirrors the local clear in ``chat_runner``:
        # empty the rows, then mark the slot dirty so the emptied transcript is
        # what gets saved. The peer's own "conversation cleared" confirmation
        # arrives as an ordinary transcript row, so nothing is appended here.
        slot.messages.clear()
        slot._dirty = True
...
async def relay_remote_turn(
    ...
    *chunks* exists for tests: pass an async byte iterator to drive the replay
    ...
    # KNOWN GAP — a message sent DURING a relayed turn is queued by the busy
    # branch of ``api_chat_send`` and never executed. The local drain lives inside
    # ``_run_chat``, which this coroutine REPLACES rather than wraps, so a
    # peer-bound slot reaches no drain at all.
    #
    # It is deliberately still a gap rather than a fix. Borrowing
    # ``_start_next_queued_turn`` was tried and REVERTED: it carries no
    # ``is_remote``/``executor`` branch and dispatches ``_run_chat``
    # unconditionally, so draining through it ran the follow-up the user aimed at
    # the crew on THIS machine — local tools, local filesystem, a locally-authored
    # answer — which is the wrong-machine execution the ``executor == "remote"``
    # guard at the primary dispatch exists to prevent, and strictly worse than
    # losing the message. Closing it properly means making the queue dispatcher
    # executor-aware (so any non-local executor is routed correctly, not just this
    # one), which is a change to the shared local turn path and a decision of its
    # own rather than a tail-call here.
